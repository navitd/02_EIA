...
print("working directory of output_multipliers_all_sectors.py is: ",os.getcwd())  # Print the current working directory

# predicting output from year1 to year2
year1 = '2015'
year2 = '2016'

_, _, _, _, _, _, _, _, T_year1, Tc_year1, Ldf_year1, _, Lcdf_year1, _ = clc_output_multipliers(year1)
...
#output multipliers - sum over rows - the sume of each column in L and Lc
mo = Ldf_year1.sum(axis=0)
moc_trancated = Lcdf_year1.iloc[:-1].sum(axis=0)

# income multipliers
Ej_by_xj = Tc_year1.iloc[-1,:-1] #hosehold income received per dollar's worth of sector output  

income_F_multipliers = Ldf_year1.mul(Ej_by_xj, axis=0) #household income recieved per dollar's worth of secotr final demand
...
print('')
print(total_income_F_multipliers)
