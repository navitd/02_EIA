...
##########################
# To use DOM:
#final demand vectors are with imports and exports. should I remove them?
#names of sectors changed to D
#rows are not labled with D and D needs to be added
# number of sectors differs, 45 vs. 50. perhaps I should just call it D. YES!

# C06 collect data 1995-2020: fHFCE, fother, Tc, output, GDPj_by_xj and everything else I may need later
# 
...
#################### compare outputs
def clc_multipliers(country, year, Ldf, Lcdf, Tc, GDP, outputc):        
...
#multipliers2prediction(Ldf, f9, "predicted output") and Ldf@f are equivalent
...
if table_type == 'DOM':
    ...
    final_demand_columns = ["HFCE","NPISH","GGFC","GFCF","INVNT","DPABR","CONS_NONRES","EXPO","IMPO"]
                            # old tables: ['HFCE',	'NPISH', 'GGFC', 'GFCF',	'INVNT', 'CONS_NONRES', 'EXPO']
    fixed_sectors = [
                    "D01","D02","D03","D05","D06","D07","D08","D09","D10T12","D13T15","D16",
                    "D17T18","D19","D20","D21","D22","D23","D24A","D24B","D25","D26","D27",
                    "D28","D29","D301","D302T309","D31T33","D35","D36T39","D41T43","D45T47",
                    "D49","D50","D51","D52","D53","D55T56","D58T60","D61","D62T63",
                    "D64T66","D68","D69T75","D77T82","D84","D85","D86T88","D90T93",
                    "D94T96","D97T98"] #50
elif table_type == 'TTL':
...
# except for D24A and D24B and perhaps others
sector_map = {
    "D01T02": "A01_02",
    "D03": "A03",
    "D05T06": "B05_06",
...
for country in countries:
    for year in year_range:
        ...
        PPP_or_exch, OECD, simple_II_labels =  data_upload_OECD_without_E(year, currency_exchange_type, table_type, country, sector_map)
...
if 1:
    dfTc.to_csv("Bench_predictions_C/C06_dfTc.csv", index=False)
    dfHFCE.to_csv("Bench_predictions_C/C06_dfHFCE.csv", index=False)
    df8.to_csv("Bench_predictions_C/C06_df8.csv", index=False)
    df9.to_csv("Bench_predictions_C/C06_df9.csv", index=False)
    dfGDP.to_csv("Bench_predictions_C/C06_dfGDP.csv", index=False)
    dfoutput.to_csv("Bench_predictions_C/C06_dfoutput.csv", index=False)
    dfGDPj_by_xj.to_csv("Bench_predictions_C/C06_dfGDPj_by_xj.csv", index=False)
    dfEj_by_xj.to_csv("Bench_predictions_C/C06_dfEj_by_xj.csv", index=False)
    
    
    #multipliers:
    dfmo.to_csv("Bench_predictions_C/C06_dfmo.csv", index=False)
    dfmoc.to_csv("Bench_predictions_C/C06_dfmoc.csv", index=False)
    dfmh.to_csv("Bench_predictions_C/C06_dfmh.csv", index=False)
    dfmhc.to_csv("Bench_predictions_C/C06_dfmhc.csv", index=False)
    dfmg.to_csv("Bench_predictions_C/C06_dfmg.csv", index=False)
    dfmgc.to_csv("Bench_predictions_C/C06_dfmgc.csv", index=False)                    
...
#plot extrapolation
#plot_v_by_year_1panel(df9_extrap_and_data_wide, countries, 'final demand [Million USD]', "Extrapolated other final demand by Country")
...
if 1:
    dfHFCE_extrap_and_data.to_csv("Bench_predictions_C/C06_dfHFCE_tot.csv", index=False)
    df8_extrap_and_data.to_csv("Bench_predictions_C/C06_df8_tot.csv", index=False)
    df9_extrap_and_data.to_csv("Bench_predictions_C/C06_df9_tot.csv", index=False)
    dfGDP_extrap_and_data.to_csv("Bench_predictions_C/C06_dfGDP_tot.csv", index=False)
    dfoutput_extrap_and_data.to_csv("Bench_predictions_C/C06_dfoutput_tot.csv", index=False)
    dfGDPj_by_xj_extrap_and_data.to_csv("Bench_predictions_C/C06_dfGDPj_by_xj_tot.csv", index=False)
    dfEj_by_xj_extrap_and_data.to_csv("Bench_predictions_C/C06_dfEj_by_xj_tot.csv", index=False)
if 0:
    #checks
    #1. GDPj_by_xj *output = GDP??
    #the above is not true. it's true only for _xj_, not the summing of output

    #2. dfHFCE+dfother = df9??
    #df9check= dfHFCE_extrap_and_data_wide + dfother_extrap_and_data_wide

    #diff = np.abs(df9check - df9_extrap_and_data_wide)
    #diff_ratio = diff/df9_extrap_and_data_wide
    #diff_mean = diff_ratio.mean()

    #mask_years = lambda df: (df.index >= 2010) & (df.index <= 2021) 
    #print('df9check - result of adding HFCE+other')
    #print(df9check[mask_years(df9check)])
    ...
    #print('         HFCE       ', ' other    ', '9 by addition ', '9 from df')
    ...
    #                 df9check[mask_years(df9check)]['CAN'], 
    #                 df9_extrap_and_data_wide[mask_years(df9_extrap_and_data_wide)]['CAN']], axis=1))
