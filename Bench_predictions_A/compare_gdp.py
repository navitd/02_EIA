...
#comparison gdp
#world bank gdp is in 2024 usd
#OECD gdp is in that year's usd millions

dfGDP_grouped = dfGDP.groupby(['country', 'year'])['GDP'].sum().unstack('country')
dfGDP_grouped.index = dfGDP_grouped.index.astype(int)
dfgdp_worldbank.index = dfgdp_worldbank.index.astype(int)
for country in countries:
    # Select the GDP series for this country (indexed by year)
    value2 = dfGDP_grouped[country].loc[int(first_year):int(last_year)]
    
    # Compute the difference with the World Bank data
    diff = dfgdp_worldbank.loc[int(first_year):int(last_year), country] - value2
