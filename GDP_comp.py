...
year = re.search(r'\d{4}', OECD_name).group()
...
'''
I don't want to print over the excel - it will delete what I have
# print to excel to see the sectors
file_path = "/mnt/c/NavitComputer24/2024_NES/Economics/Data/Navit_map_check.xlsx"
sheet_name = "Sheet1"

# Load the workbook
with pd.ExcelWriter(file_path, engine="openpyxl", mode="a", if_sheet_exists="overlay") as writer:
    temp = GDP.reset_index(name='GDP_values')
    temp.rename(columns={'index': 'codes'}, inplace=True)
    temp.codes.to_excel(writer, sheet_name=sheet_name, startcol=4, index=False, header=False)
'''
